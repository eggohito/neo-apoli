...
import io.github.eggohito.neo_apoli.action.meta.block.ReferenceBlockAction;
...
public interface BlockAction extends Action<BlockActionType<?>> {
	...
	Codec<BlockAction> CODEC = Codec.recursive("BlockAction", codec -> new MultiAlternativeCodec<>(BlockActionTypes.CODEC.dispatch(TYPE_KEY, BlockAction::getType, BlockActionType::mapCodec), Identifier.CODEC.xmap(ReferenceBlockAction::new, ReferenceBlockAction::value), codec.listOf().xmap(SequenceBlockAction::new, SequenceBlockAction::actions)));
	...
	default String asDisplayString() {
		return ActionManager.getIdAsResult(this)
			.result()
			.map(id -> "Block action with ID \"" + id + "\"")
			.orElseGet(() -> "Block action with type \"" + RegistryUtil.getId(NeoApoliRegistries.BLOCK_ACTION_TYPE, this.getType()) + "\"");
	}
