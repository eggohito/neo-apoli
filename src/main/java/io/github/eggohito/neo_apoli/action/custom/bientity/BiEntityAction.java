...
public interface BiEntityAction extends Action {
	...
	Codec<BiEntityAction> CODEC = Codec.recursive(BiEntityAction.class.getSimpleName(), codec -> new MultiAlternativeCodec<>(Type.CODEC.dispatch(BiEntityAction::getType, Type::mapCodec), codec.listOf().xmap(SequenceBiEntityAction::new, SequenceBiEntityAction::actions)));

	StreamCodec<RegistryFriendlyByteBuf, BiEntityAction> STREAM_CODEC = Type.STREAM_CODEC.dispatch(BiEntityAction::getType, Type::streamCodec);
	...
	enum Kind implements Action.Kind<BiEntityAction> {
	...
	record Type<A extends BiEntityAction>(MapCodec<A> mapCodec, StreamCodec<RegistryFriendlyByteBuf, A> streamCodec) implements Action.Type<A> {

		public static final FixedRegistryAlias<Type<?>> ALIASES = FixedRegistryAlias.extended(NeoApoliRegistries.BIENTITY_ACTION_TYPE, Action.Type.ALIASES);

		public static final Codec<Type<?>> CODEC = ALIASES.createCodec(NeoApoli.MOD_NAMESPACE);

		public static final StreamCodec<RegistryFriendlyByteBuf, Type<?>> STREAM_CODEC = ByteBufCodecs.registry(NeoApoliRegistryKeys.BIENTITY_ACTION_TYPE);

		@Override
		public BiEntityAction.Kind kind() {
