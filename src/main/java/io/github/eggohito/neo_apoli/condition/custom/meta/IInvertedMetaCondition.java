...
public interface IInvertedMetaCondition<C extends Condition> extends MetaCondition {
	...
	static <C extends Condition, M extends IInvertedMetaCondition<C>> MapCodec<M> createCodec(Codec<C> conditionCodec, Function<C, M> constructor) {
		return RecordCodecBuilder.mapCodec(instance -> instance.group(
			conditionCodec.fieldOf("condition").forGetter(IInvertedMetaCondition::condition)
		).apply(instance, constructor));
	}

	static <C extends Condition, M extends IInvertedMetaCondition<C>> StreamCodec<RegistryFriendlyByteBuf, M> createStreamCodec(StreamCodec<RegistryFriendlyByteBuf, C> conditionCodec, Function<C, M> constructor) {
