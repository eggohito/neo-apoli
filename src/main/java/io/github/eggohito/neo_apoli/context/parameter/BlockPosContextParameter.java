...
public class BlockPosContextParameter extends ContextParameter<BlockPos> {

	public BlockPosContextParameter(ResourceLocation name) {
		super(name);
	}

	@Override
	public @NotNull Class<BlockPos> typeClass() {
	...
	public void addAsArgument(CommandBuildContext buildContext, CommandNode<CommandSourceStack> baseNode, CommandNode<CommandSourceStack> parameterNode) {
		...
		CommandNode<CommandSourceStack> posNode = argument("pos", BlockPosArgument.blockPos())
			.redirect(baseNode, this::addPosToContext)
			.build();
	...
	protected CommandSourceStack addPosToContext(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {

		CommandSourceStack source = context.getSource();
		BlockPos blockPos = BlockPosArgument.getLoadedBlockPos(context, "pos");

		((ContextBuilderHolder) source).neo_apoli$getContextBuilder().withRequired(this, blockPos);
		return source;
