...
public class ItemStackContextParameter extends TypedContextParameter<ItemStack> {

	public ItemStackContextParameter(Identifier id) {
		super(id, ItemStack.class);
	}
		...
		return new CommandBuilder() {
			...
			ServerCommandSource redirectBlock(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
				...
				if (world.getBlockEntity(blockPos) instanceof Inventory inventory) {
					...
					if (slot >= 0 && slot < inventory.size()) {
						((ServerContextBuilderHolder) source).neo_apoli$getBuilder().add(ItemStackContextParameter.this, inventory.getStack(slot));
					}
			...
			ServerCommandSource redirectEntity(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
				...
				if (stackReference != StackReference.EMPTY) {
					((ServerContextBuilderHolder) source).neo_apoli$getBuilder().add(ItemStackContextParameter.this, stackReference.get());
				}
